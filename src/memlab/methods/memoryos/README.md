...
```mermaid
flowchart TB
    STM["<b>STM</b> — dialogue page 큐 (7칸)<br/>page = {Q, R, T} + meta_chain"]
    MTM["<b>MTM</b> — 주제별 segment (최대 200개)<br/>heat가 삭제·승격을 결정"]
    LPM["<b>LPM</b> — dual persona (영구 저장)<br/>User Profile(90차원)·KB(100칸) / Agent Profile·Traits(100칸)"]
    GONE(["삭제됨"])

    STM -->|"큐가 차면 가장 오래된 page를 FIFO 이관"| MTM
    MTM -->|"heat > τ=5 승격 (이후 L ← 0 리셋)"| LPM
    MTM -.->|"자리 부족 시 heat 최저 segment 삭제"| GONE
```

| 식 | 무엇 | 상수 |
|---|---|---|
| (1) `page_chain = {Q, R, T, meta_chain}` | page마다 대화 흐름 요약 — LLM이 ① 이어짐 판단 ② chain 전체 재요약 | — |
| (2)(3) `F_score = cos(e_s, e_p) + Jaccard(K_s, K_p)` | segment 편입 기준 — 임계값을 넘는 최적 segment에 합류, 없으면 신설 | θ = 0.6 |
| (4) `Heat = α·N_visit + β·L_interaction + γ·R_recency` | 검색에 걸리면 N_visit 증가 → heat 상승. 최저 heat는 삭제, τ를 넘으면 승격 | α=β=0.8, γ=0.0001, τ=5 |
